...
class MainWindow(QWidget):
	def __init__(self, parent = None):
		...
		# Radio Button
		rb1 = QRadioButton('Hill Algorithm')
		rb1.setFont(font)
		rb1.setChecked(True)
		grid.addWidget(rb1, 2, 1)
		rb2 = QRadioButton('Simulated Annealing')
		rb2.setFont(font)
		grid.addWidget(rb2, 3, 1)
		rb3 = QRadioButton('Genetic Algorithm')
		rb3.setFont(font)
		grid.addWidget(rb3, 4, 1)
		
		# Open File Button (TBD)
		fopenbut = QPushButton('Open File...')
		grid.addWidget(fopenbut, 1, 0)
		QObject.connect(fopenbut, SIGNAL("clicked()"), self.choose_file)

		# Label Misc.
		choosealg = QLabel('Choose Method:')
		choosealg.setFont(font)
		grid.addWidget(choosealg, 1, 1)
		consbroken = QLabel('Constraint Broken:')
		consbroken.setFont(font)
		grid.addWidget(consbroken, 5, 2)
		
		# Define Main Table
		table = QTableWidget()
		table.setRowCount(11)
		table.setColumnCount(5)
		table.setHorizontalHeaderLabels(['Senin','Selasa','Rabu','Kamis','Jumat'])
		table.horizontalHeader().setResizeMode(QHeaderView.Stretch)
		table.verticalHeader().setResizeMode(QHeaderView.Stretch)
		table.setVerticalHeaderLabels(['07:00','08:00','09:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00'])
		...
		# Run Push Button
		runbut = QPushButton('Run!')
		grid.addWidget(runbut, 5, 1)
		QObject.connect(runbut, SIGNAL("clicked()"), self.setTableContentGA)
		
		
	# Fill the Table with Genetic Algorithm Data
		...
		for i in range (len(varMatkul[0])):
			...
			if (varMatkul[0][i].hari == 1):
				for j in range (0, lama):
					...
					table.setItem(start, 0, QTableWidgetItem(soda))
					start = start+1
			elif (varMatkul[0][i].hari == 2):
				for j in range (0,lama):
					soda = varMatkul[0][i].kode + '  -  ' + varMatkul[0][i].kelas
					table.setItem(start, 1, QTableWidgetItem(soda))
					start = start+1
			elif (varMatkul[0][i].hari == 3):
				for j in range (0,lama):
					soda = varMatkul[0][i].kode + '  -  ' + varMatkul[0][i].kelas
					table.setItem(start, 2, QTableWidgetItem(soda))
					start = start+1
			elif (varMatkul[0][i].hari == 4):
				for j in range (0,lama):
					soda = varMatkul[0][i].kode + '  -  ' + varMatkul[0][i].kelas
					table.setItem(start, 3, QTableWidgetItem(soda))
					start = start+1
			elif (varMatkul[0][i].hari == 5):
				for j in range (0,lama):
					soda = varMatkul[0][i].kode + '  -  ' + varMatkul[0][i].kelas
					table.setItem(start, 4, QTableWidgetItem(soda))
					start = start+1
	...
	# Algorithm Choosing, Run Button Clicked
	def runClicked(self):
		if radiobutton.text() == "Hill Algorithm":
			if radiobutton.isChecked() == True:
				pass
			else:
				pass
		elif radiobutton.text() == "Simulated Annealing":
			if radiobutton.isChecked() == True:
				pass
			else:
				pass
		elif radiobutton.text() == "Genetic Algorithm":
			if radiobutton.isChecked() == True:
